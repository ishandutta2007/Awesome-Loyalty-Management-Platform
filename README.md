...
```mermaid
flowchart TD
    A["Customer Event (Purchase/Referral)"] --> B["API Gateway"]
    B --> C["Rules & Qualification Engine"]
    C --> D{"Eligible?"}
    D -- Yes --> E["Loyalty Ledger (Points Credit/Debit)"]
    D -- No --> F["Log Ineligible Event"]
    E --> G["Tier Processing Engine"]
    E --> H["Kafka Event Stream"]
    H --> I["Notification Engine (Novu/Mautic)"]
    H --> J["Real-time Analytics (ClickHouse)"]
```
